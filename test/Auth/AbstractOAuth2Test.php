...
class AbstractOAuth2Test extends TestCase
{
    ...
    {
        $this->auth->setScopes(['data:read', 'data:write']);
        $this->assertEquals(['data:read', 'data:write'], $this->auth->getScopes());

        $this->auth->setScopes(['data:read']);
        $this->assertEquals(['data:read'], $this->auth->getScopes());
    }
//
